...
class SemesterReport extends Component {
  state = {
    loading: true,
    callUpdate: false,
    updating: false,
  ...
  async componentDidUpdate() {
    ...
    if (this.state.callCreateReport) {
      ...
      await Fetch.fetchSemesterReport(this, ['semesterID', 'gradeID', 'classID']);
  ...
  render() {
    return (
      this.state.loading ? <LoadScreen style={{height: 'calc(100vh - 350px)'}} /> : (
        <div className={[styles.SemesterReport, styles.FadeWrapper].join(' ')}>
            ...
            <div>
              <Button
                ...
                disabled={
                  ...
                  !(this.state.filters.year && this.state.filters.semesterID && this.state.filters.gradeID)
                }
                style={{
                  height: '32px',
                  marginBottom: '32px',
